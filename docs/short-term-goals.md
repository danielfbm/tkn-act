...
| 9 | **Resolvers** (git / hub / cluster / bundles) | Largest user-impact item — most catalog usage references `taskRef` from a hub. | Phase 1 (types + lazy-dispatch + cluster inline + validator + events + CLI flag scaffolding), Phase 2 (direct `git` resolver via go-git/v5; cross-backend `resolver-git` e2e fixture), and Phase 3 (`hub` + `http` direct resolvers; cross-backend e2e fixtures `resolver-http`, `resolver-hub`) shipped in v1.6.x; remaining phases (`bundles` + `cluster` Phase 4, remote ResolutionRequest driver Phase 5, offline polish Phase 6) tracked in `docs/superpowers/plans/2026-05-04-resolvers.md`. |
