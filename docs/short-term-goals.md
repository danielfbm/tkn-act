...
| 9 | **Resolvers** (git / hub / cluster / bundles / remote) | Largest user-impact item — most catalog usage references `taskRef` from a hub. | Phase 1 (types + lazy-dispatch + cluster inline + validator + events + CLI flag scaffolding), Phase 2 (direct `git` resolver via go-git/v5; cross-backend `resolver-git` e2e fixture), Phase 3 (`hub` + `http` direct resolvers; cross-backend e2e fixtures `resolver-http`, `resolver-hub`), Phase 4 (direct `bundles` resolver via go-containerregistry + off-by-default direct `cluster` resolver via the kube dynamic client; cross-backend `resolver-bundles` e2e fixture; new `--cluster-resolver-context` / `--cluster-resolver-kubeconfig` flags), and Phase 5 (Mode B remote resolver via `resolution.tekton.dev/v1beta1.ResolutionRequest` CRD with v1alpha1 fallback; new `--remote-resolver-context` / `--remote-resolver-namespace` / `--remote-resolver-timeout` flags; `resolver-remote` cluster e2e fixture; `defer Delete(...)` via `context.Background()` so SIGINT mid-resolution still triggers cleanup) shipped in v1.6.x; remaining phase (offline polish Phase 6) tracked in `docs/superpowers/plans/2026-05-04-resolvers.md`. |
