...
| Resolvers — git / hub / cluster / bundles / remote | `taskRef.resolver` | in-progress | both | resolver-git, resolver-http, resolver-hub, resolver-bundles, resolver-remote | none | docs/superpowers/plans/2026-05-04-resolvers.md (Track 1 #9) — Phase 1 (types + lazy-dispatch + cluster inline + validator + events + CLI flags), Phase 2 (direct `git` resolver via go-git/v5), Phase 3 (`hub` + `http` direct resolvers), Phase 4 (`bundles` via go-containerregistry + `cluster` off-by-default direct resolver), and Phase 5 (Mode B remote resolver via `resolution.tekton.dev/v1beta1` `ResolutionRequest` CRD with v1alpha1 fallback; arbitrary custom resolver names are valid in Mode B) shipped with cross-backend e2e fixtures; offline cache polish (Phase 6) tracked in the plan |
