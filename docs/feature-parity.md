...
| Resolvers — git / hub / cluster / bundles | `taskRef.resolver` | in-progress | both | resolver-git, resolver-http, resolver-hub, resolver-bundles | none | docs/superpowers/plans/2026-05-04-resolvers.md (Track 1 #9) — Phase 1 (types + lazy-dispatch + cluster inline + validator + events + CLI flags), Phase 2 (direct `git` resolver via go-git/v5), Phase 3 (`hub` + `http` direct resolvers), and Phase 4 (`bundles` via go-containerregistry + `cluster` off-by-default direct resolver) shipped with cross-backend e2e fixtures; remote ResolutionRequest driver (Phase 5) and offline cache polish (Phase 6) tracked in the plan |
